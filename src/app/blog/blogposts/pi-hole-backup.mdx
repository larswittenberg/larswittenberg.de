...
# Alte Backups löschen, um Platz zu sparen
# Alle .zip-Dateien nach Datum sortiert auflisten, die neuesten 8 überspringen und den Rest löschen, um Platz zu sparen.
ls -t "$BACKUP_DIR"/*.zip 2>/dev/null | tail -n +9 | xargs -I {} rm {}
```
...
In diesem Schritt legen wir noch eine lokale Funktion an, damit du die neuste Backup-Datei einfach auf deinen Mac herunterladen kannst. Trenne dazu die SSH-Verbindung zum Pi-Hole mit `exit` und öffne die Terminal-Konfiguration auf deinem Mac (bei mir ist das die Zsh-Konfiguration):

```bash
nano ~/.zshrc
```

Füge am Ende der Datei folgende Funktion hinzu:

```bash
get-pihole-backup() {
    # 1. Holt den neuesten Dateinamen direkt über SSH vom Pi
    NEUESTE_DATEI=$(ssh pihole "ls -t pihole-backups/*.zip 2>/dev/null | head -n 1")

    if [ -z "$NEUESTE_DATEI" ]; then
        echo "Keine Backup-Dateien auf dem Pi gefunden!"
        return 1
    fi

    # 2. Lädt genau diese Datei herunter
    echo "Lade neueste Datei herunter: $(basename "$NEUESTE_DATEI")"
    scp "pihole:$NEUESTE_DATEI" ~/Desktop/
}
```
...
Jetzt kannst du einfach `get-pihole-backup` in deinem Terminal eingeben, um das neuste Backup von deinem Pi-Hole auf deinen Mac auf den Schreibtisch (Desktop-Ordner) herunterzuladen. Statt "~/Desktop/" kannst du natürlich auch einen anderen Pfad angeben, wenn du die Datei woanders speichern möchtest.
